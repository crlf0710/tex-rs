...
macro_rules! start_font_error_message {
    ($globals:expr, $nom:expr, $aire:expr, $s:expr) => {{
        // print_char("="); print_file_name(nom,aire,"");
        print_char(make_globals_io_string_log_view!($globals), ASCII_code_literal!(b'='));
        print_file_name($globals, $nom.get() as _, $aire.get() as _, strpool_str!("").get() as _);
        // if s>=0 then
        if $s.inner() >= 0 {
        ...
        // else if s<>-1000 then
            ...
            print($globals, strpool_str!(" scaled ").get() as _);
...
macro_rules! Report_that_the_font_wont_be_loaded {
    ($globals:expr, $nom:expr, $aire:expr, $s:expr, $file_opened:expr) => {{
        ...
        if $file_opened {
            print($globals, strpool_str!(" not loadable: Bad metric (TFM) file").get() as _);
        }
        // else print(" not loadable: Metric (TFM) file not found");
        else {
            print($globals, strpool_str!(" not loadable: Metric (TFM) file not found").get() as _);
        }
