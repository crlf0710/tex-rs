...
macro_rules! If_all_characters_of_the_family_fit_relative_to_h__then_goto_found__otherwise_goto_not_found {
    ($globals:expr, $h:expr, $p:expr, $lbl_not_found:lifetime, $lbl_found:lifetime) => {{
        ...
        while q > 0 {
            ...
            {
                goto_forward_label!($lbl_not_found);
            }
            // q:=trie_r[q];
            q = $globals.trie_r[q];
            // end;
