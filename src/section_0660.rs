...
macro_rules! Report_an_underfull_hbox_and_goto_common_ending__if_this_box_is_sufficiently_bad {
    ($globals:expr, $x:expr, $lbl_common_ending:lifetime) => {{
        // begin last_badness:=badness(x,total_stretch[normal]);
        $globals.last_badness =
            badness($globals, $x, $globals.total_stretch[glue_ord::normal]) as _;
        ...
        if $globals.last_badness > hbadness!($globals) {
            ...
            print($globals, strpool_str!(" \\hbox (badness ").get() as _);
