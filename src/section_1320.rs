...
macro_rules! Dump_the_font_information {
    ($globals:expr) => {{
        // dump_int(fmem_ptr);
        ...
        for k in null_font.get()..= $globals.font_ptr.get() {
            ...
            Dump_the_array_info_for_internal_font_number_k!($globals, k);
        }
        ...
        print($globals, strpool_str!(" words of font info for ").get() as _);
        // print_int(font_ptr-font_base); print(" preloaded font");
        print_int($globals, $globals.font_ptr.get() as integer - font_base as integer);
        print($globals, strpool_str!(" preloaded font").get() as _);
