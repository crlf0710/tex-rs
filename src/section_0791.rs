...
pub(crate) fn fin_col(globals: &mut TeXGlobals) -> TeXResult<boolean> {
    ...
    if globals.cur_align == null {
        confusion(globals, strpool_str!("endv"))?;
    }
    ...
    if globals.align_state < 500000 {
        ...
        fatal_error(globals, strpool_str!("(interwoven alignment preambles are not allowed)"))?;
    }
    // @.interwoven alignment preambles...@>
    // p:=link(q);
    p = link!(globals, q);
    // @<If the preamble list has been traversed, check that the row has ended@>;
    If_the_preamble_list_has_been_traversed__check_that_the_row_has_ended!(globals, p);
    ...
    if extra_info!(globals, globals.cur_align) != span_code {
        ...
        Package_an_unset_box_for_the_current_column_and_record_its_width!(globals);
        // @<Copy the tabskip glue between columns@>;
        Copy_the_tabskip_glue_between_columns!(globals);
