...
macro_rules! undump {
    ($globals:expr, $min:expr, $max:expr, $val:expr, $val_ctor:expr, $lbl_bad_fmt:lifetime) => {{
        let x;
        undump_int!($globals, x);
        if x < $min as integer || x > $max as integer {
            trace_error_expr!("{} is not in {}..={}", x, $min, $max);
...
macro_rules! undump_size {
    ($globals:expr, $min:expr, $max:expr, $prompt:expr, $val:expr, $val_ctor:expr, $lbl_bad_fmt:lifetime) => {{
        let x;
        undump_int!($globals, x);
        if x < $min as integer {
            goto_forward_label!($lbl_bad_fmt);
