...
pub(crate) fn put_each_of_tex_s_primitivies_into_the_hash_table_0491(globals: &mut TeXGlobals) {
    ...
    primitive(globals, strpool_str!("fi"), fi_or_else, fi_code as _);
    // @!@:fi_}{\.{\\fi} primitive@>
    // text(frozen_fi):="fi"; eqtb[frozen_fi]:=eqtb[cur_val];
    text!(globals, frozen_fi as pointer) = strpool_str!("fi").get() as _;
    globals.eqtb[frozen_fi as pointer] = globals.eqtb[globals.cur_val as pointer];
    // primitive("or",fi_or_else,or_code);
    primitive(globals, strpool_str!("or"), fi_or_else, or_code as _);
    // @!@:or_}{\.{\\or} primitive@>
    // primitive("else",fi_or_else,else_code);
    primitive(globals, strpool_str!("else"), fi_or_else, else_code as _);
