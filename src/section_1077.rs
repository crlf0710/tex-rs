...
macro_rules! Store_cur_box_in_a_box_register {
    ($globals:expr, $box_context:expr) => {{
        ...
        if $box_context < box_flag + 256 {
            ...
            eq_define($globals, box_base as pointer + ($box_context - box_flag) as pointer, box_ref, $globals.cur_box)
        }
        // else geq_define(box_base-box_flag-256+box_context,box_ref,cur_box)
        else {
            geq_define($globals, box_base as pointer + ($box_context - box_flag - 256) as pointer, box_ref, $globals.cur_box)
        }
