//! ` `
// @<Cases of |handle...@>=
macro_rules! Cases_of_handle_right_brace_where_a_right_brace_triggers_a_delayed_action_1100 {
    ($globals:expr) => {{
        // insert_group: begin end_graf; q:=split_top_skip; add_glue_ref(q);
        if $globals.cur_group == insert_group {
        ...
        // output_group: @<Resume the page builder...@>;
            ...
            Resume_the_page_builder_after_an_output_routine_has_come_to_an_end!($globals);
            true
        } else {
            false
        }
