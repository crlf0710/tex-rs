...
macro_rules! Select_the_appropriate_case_and_return_or_goto_common_ending {
    ($globals:expr, $save_cond_ptr:expr, $lbl_common_ending:lifetime) => {{
        ...
        if tracing_commands!($globals) > 1 {
            ...
            print($globals, strpool_str!("{case ").get() as _);
            print_int($globals, n);
