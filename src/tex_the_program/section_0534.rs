...
pub(crate) fn open_log_file(globals: &mut TeXGlobals) {
    ...
    if globals.job_name == 0 {
        globals.job_name = crate::strpool_str!("texput");
    }
    // @.texput@>
    // pack_job_name(".log");
    ...
    globals.log_name =
        a_make_name_string(make_globals_io_string_view!(globals), &mut globals.log_file);
    // selector:=log_only; log_opened:=true;
    globals.selector = log_only.into();
    globals.log_opened = true;
    // @<Print the banner line, including the date and time@>;
    crate::section_0536::Print_the_banner_line__including_the_date_and_time!(globals);
