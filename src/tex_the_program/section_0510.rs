...
pub(crate) macro Terminate_the_current_conditional_and_skip_to_fi($globals:expr) {{
    ...
    if $globals.cur_chr.get() > $globals.if_limit.get() as _ {
        ...
        //   else  begin print_err("Extra "); print_cmd_chr(fi_or_else,cur_chr);
            ...
            print_err!($globals, crate::strpool_str!("Extra "));
            ...
            help1!(
                $globals,
                crate::strpool_str!("I'm ignoring this; it doesn't match any \\if.")
            );
            // error;
            error($globals)?;
            // end
