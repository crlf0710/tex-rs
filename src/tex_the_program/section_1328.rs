...
pub(crate) macro Create_the_format_ident__open_the_format_file__and_inform_the_user_that_dumping_has_begun($globals:expr) {{
    // selector:=new_string;
    $globals.selector = new_string.into();
    // print(" (preloaded format="); print(job_name); print_char(" ");
    print(
        $globals,
        crate::strpool_str!(" (preloaded format=").get() as _,
    );
    ...
    pack_job_name($globals, crate::strpool_str!(format_extension));
    ...
    while !w_open_out(
        ...
        prompt_file_name(
            $globals,
            crate::strpool_str!("format file name"),
            crate::strpool_str!(format_extension),
        )?;
    }
    // print_nl("Beginning to dump on file ");
    print_nl($globals, crate::strpool_str!("Beginning to dump on file "));
    // @.Beginning to dump...@>
