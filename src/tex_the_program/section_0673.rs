...
pub(crate) macro Determine_vertical_glue_stretch_setting__then_return_or_goto_common_ending($globals:expr, $r:expr, $x:expr, $lbl_common_ending:lifetime) {{
    /// order of infinity
    let o: glue_ord;
    ...
    if o == glue_ord::normal && list_ptr!($globals, $r) != null {
        ...
        //   is sufficiently bad@>;
        crate::section_0674::Report_an_underfull_vbox_and_goto_common_ending__if_this_box_is_sufficiently_bad!(
            $globals,
            $x,
            $lbl_common_ending
        );
