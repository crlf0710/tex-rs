...
// @<Display token |p|...@>=
pub(crate) macro Display_token_p__and_return_if_there_are_problems($globals:expr, $p:expr, $n:expr, $match_chr:expr) {{
    // if (p<hi_mem_min) or (p>mem_end) then
    if ($p as pointer) < $globals.hi_mem_min || ($p as pointer) > $globals.mem_end {
        crate::trace_error_expr!("clobbered p = {}", $p as pointer);
    ...
    // else  begin m:=info(p) div @'400; c:=info(p) mod @'400;
        ...
        if info_p.get() < 0 {
            print_esc($globals, crate::strpool_str!("BAD."));
        }
        // @.BAD@>
        //   else @<Display the token $(|m|,|c|)$@>;
        else {
            crate::section_0294::Display_the_token_m_c!($globals, m, c, $n, $match_chr);
        }
        // end
    }
