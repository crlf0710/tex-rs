...
macro_rules! Move_node_p_to_the_current_page__if_it_is_time_for_a_page_break__put_the_nodes_following_the_break_back_onto_the_contribution_list__and_return_to_the_user_s_output_routine_if_there_is_one {
    ($globals:expr, $p:expr, $lbl_continue:lifetime) => { #[allow(unused_assignments)] {
        ...
        Check_if_node_p_is_a_new_champion_breakpoint__then_if_it_is_time_for_a_page_break__prepare_for_output__and_either_fire_up_the_user_s_output_routine_and_return_or_ship_out_the_page_and_goto_done!
            ($globals, $p, pi, 'done);
        ...
        Update_the_current_page_measurements_with_respect_to_the_glue_or_kern_specified_by_node_p!
            ($globals, $p);
        }
        // contribute: @<Make sure that |page_max_depth| is not exceeded@>;
        'contribute <-
        );
        Make_sure_that_page_max_depth_is_not_exceeded!($globals);
        // @<Link node |p| into the current page and |goto done|@>;
        Link_node_p_into_the_current_page_and_goto_done!($globals, $p, 'done);
        }
