...
pub(crate) macro Output_node_p_for_vlist_out_and_move_to_the_next_node__maintaining_the_condition_cur_h_eq_left_edge {
    ($globals:expr, $p:expr, $left_edge:expr, $top_edge:expr, $this_box:expr, $cur_glue:expr, $cur_g:expr, $g_sign:expr, $g_order:expr) => {{
        ...
        |'next_p|
        {
            ...
            // else @<Output the non-|char_node| |p| for |vlist_out|@>;
                ...
                crate::section_0631::Output_the_non_char_node_p_for_vlist_out!($globals, $p, $left_edge, $top_edge, $this_box, $cur_glue, $cur_g, $g_sign, $g_order, 'next_p);
            }
        }
        // next_p:p:=link(p);
        'next_p <-
