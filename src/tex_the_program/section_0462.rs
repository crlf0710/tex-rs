...
pub(crate) macro Create_a_new_glue_specification_whose_width_is_cur_val__scan_for_its_stretch_and_shrink_components($globals:expr, $mu:expr) {{
    ...
    if scan_keyword($globals, crate::strpool_str!("plus"))? {
        // @.plus@>
    ...
    if scan_keyword($globals, crate::strpool_str!("minus"))? {
        // @.minus@>
        // begin scan_dimen(mu,true,false);
        scan_dimen($globals, $mu, true, false)?;
