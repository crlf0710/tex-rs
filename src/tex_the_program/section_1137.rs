...
pub(crate) macro Cases_of_main_control_that_build_boxes_and_lists_1137($globals:expr, $abs_mode_plus_cur_cmd:expr) {{
    // hmode+math_shift:init_math;
    let processed = if $abs_mode_plus_cur_cmd == hmode as u16 + math_shift as u16 {
        init_math($globals)?;
