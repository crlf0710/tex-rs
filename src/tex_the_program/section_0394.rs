...
macro_rules! Advance_r__goto_found_if_the_parameter_delimiter_has_been_fully_matched__otherwise_goto_continue {
    ($globals:expr, $r:expr, $info_r:expr, $lbl_found:lifetime, $lbl_continue:lifetime) => {{
        // begin r:=link(r);
        $r = link!($globals, $r);
