...
pub(crate) macro Grow_more_variable_size_memory_and_goto_restart($globals:expr, $p:expr, $q:expr, $lbl_restart:lifetime) {{
    /// temporary register
    let mut t: integer;
    ...
    // {|lo_mem_max+2<=t<hi_mem_min|}
        ...
        t = ($globals.lo_mem_max + 1 + ($globals.hi_mem_min - $globals.lo_mem_max) / 2) as integer;
        /// `lo_mem_max + 2 <= t < hi_mem_min`
        const _: () = ();
    }
