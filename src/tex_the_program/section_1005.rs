...
pub(crate) macro Check_if_node_p_is_a_new_champion_breakpoint__then_if_it_is_time_for_a_page_break__prepare_for_output__and_either_fire_up_the_user_s_output_routine_and_return_or_ship_out_the_page_and_goto_done($globals:expr, $p:expr, $pi:expr, $lbl_done:lifetime) {{
    ...
    if $pi < inf_penalty {
        ...
        crate::section_1007::Compute_the_badness_b__of_the_current_page__using_awful_bad_if_the_box_is_too_full!(
            $globals, b
        );
        // if b<awful_bad then
        if b < awful_bad {
