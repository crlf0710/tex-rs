// ` `
// @d app_lc_hex(#)==l:=#;
pub(crate) macro app_lc_hex($globals:expr, $v:expr) {{
    let l = $v;
...
// @<Make the first 256...@>=
pub(crate) macro Make_the_first_256_strings($globals:expr, $g: expr) {{
    let mut globals = make_globals_string_view!($globals);
    // for k:=0 to 255 do
    for k in 0..=255 {
        // begin if (@<Character |k| cannot be printed@>) then
        if crate::section_0049::Character_k_cannot_be_printed!(k) {
