...
pub(crate) fn get_node(globals: &mut TeXGlobals, s: integer) -> TeXResult<pointer> {
    ...
    loop {
        crate::section_0127::Try_to_allocate_within_node_p_and_its_physical_successors_and_goto_found_if_allocation_was_possible!
    ...
    if s == 0o10000000000 {
        ...
        crate::return_nojump!(max_halfword);
        // end;
    }
    ...
    if globals.lo_mem_max + 2 < globals.hi_mem_min && globals.lo_mem_max + 2 <= mem_bot + max_halfword {
        // @<Grow more variable-size memory and |goto restart|@>;
         crate::section_0126::Grow_more_variable_size_memory_and_goto_restart!(globals, p, q, 'restart);
    }
    // overflow("main memory size",mem_max+1-mem_min);
    //   {sorry, nothing satisfactory is left}
    /// sorry, nothing satisfactory is left
    overflow(globals, crate::strpool_str!("main memory size"), (mem_max + 1 - mem_min) as _)?;
