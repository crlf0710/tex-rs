...
pub(crate) macro bad_pool($globals:expr, $get_strings_started:expr, $val:expr) {
...
pub(crate) macro Read_the_other_strings_from_the_TEX_POOL_file_and_return_true_or_give_an_error_message_and_return_false {
    ($globals:expr, $get_strings_started:expr, $g:expr) => {
        crate::trace_span!("Read the other strings...");
        ...
        if a_open_in(make_globals_filename_view!($globals), &mut $globals.pool_file) {
            ...
            loop {
                crate::section_0052::Read_one_string_but_return_false_if_the_string_memory_space_is_getting_too_tight_for_comfort!
                    ($globals, c, $g);
                // until c;
                if c {
                    break;
