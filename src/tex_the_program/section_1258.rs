...
pub(crate) macro Scan_the_font_size_specification($globals:expr, $s:expr) {{
    ...
    if scan_keyword($globals, crate::strpool_str!("at"))? {
        crate::section_1259::Put_the_positive_at_size_into_s!($globals, $s);
        // @.at@>
    }
    // else if scan_keyword("scaled") then
    else if scan_keyword($globals, crate::strpool_str!("scaled"))? {
        // @.scaled@>
        ...
        if $globals.cur_val <= 0 || $globals.cur_val > 32768 {
            ...
            print_err!(
                $globals,
                crate::strpool_str!("Illegal magnification has been changed to 1000")
            );
            // @.Illegal magnification...@>
            // help1("The magnification ratio must be between 1 and 32768.");
            help1!(
                $globals,
                crate::strpool_str!("The magnification ratio must be between 1 and 32768.")
            );
