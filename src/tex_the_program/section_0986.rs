...
pub(crate) macro Show_the_status_of_the_current_page($globals:expr) {{
    // if page_head<>page_tail then
    if page_head != $globals.page_tail {
        // begin print_nl("### current page:");
        print_nl($globals, crate::strpool_str!("### current page:"));
        // if output_active then print(" (held over for next output)");
        if $globals.output_active {
            print(
                $globals,
                crate::strpool_str!(" (held over for next output)").get() as _,
            );
        }
        ...
        if $globals.page_contents > page_contents_kind::empty {
            ...
            print_nl($globals, crate::strpool_str!("total height "));
            ...
            while r != page_ins_head {
                ...
                print_int($globals, t);
                print($globals, crate::strpool_str!(" adds ").get() as _);
                ...
                if r#type!($globals, r) as integer == page_ins_node_subtype::split_up as integer {
                    ...
                    print_int($globals, t);
                    print($globals, crate::strpool_str!(" might split").get() as _);
                    // end;
                }
                // r:=link(r);
                r = link!($globals, r);
