...
pub(crate) fn get_next(globals: &mut TeXGlobals) -> TeXResult<()> {
    ...
    {
    globals.cur_cs = 0;
    crate::trace_expr_verbose!("state = {:?}", state!(globals));
    // if state<>token_list then
    if state!(globals) != token_list {
        // @<Input from external file, |goto restart| if no input found@>
        crate::section_0343::Input_from_external_file__goto_restart_if_no_input_found!(globals, 'restart);
    } else {
        // else @<Input from token list, |goto restart| if end of list or
        //   if a parameter needs to be expanded@>;
        crate::section_0357::Input_from_token_list__goto_restart_if_end_of_list_or_if_a_parameter_needs_to_be_expanded!(
            globals,
            'restart
        );
    }
    // @<If an alignment entry has just ended, take appropriate action@>;
    crate::section_0342::If_an_alignment_entry_has_just_ended_take_appropriate_action!(globals, 'restart);
    }
    |'restart|
    }
    }
