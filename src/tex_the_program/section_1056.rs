...
pub(crate) macro Cases_of_main_control_that_build_boxes_and_lists_1056($globals:expr, $abs_mode_plus_cur_cmd:expr) {{
    // vmode+hrule,hmode+vrule,mmode+vrule: begin tail_append(scan_rule_spec);
    let processed = if $abs_mode_plus_cur_cmd == vmode as u16 + hrule as u16
