...
macro_rules! Report_an_overfull_hbox_and_goto_common_ending__if_this_box_is_sufficiently_bad {
    ($globals:expr, $x:expr, $q:expr, $lbl_common_ending:lifetime) => {{
        // if (-x-total_shrink[normal]>hfuzz)or(hbadness<100) then
        if -$x - $globals.total_shrink[glue_ord::normal] > hfuzz!($globals)
            || hbadness!($globals) < 100
        {
