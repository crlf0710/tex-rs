...
pub(crate) macro Compute_the_hash_code_h($globals:expr, $h:expr, $j:expr, $l:expr) {
    ...
    {
        let mut fss_utf_bytes = ($j..=$j + $l - 1)
            .flat_map(|k| FssUtfEncodedIP32::new($globals.buffer[k as u16].0 as i32).into_iter());
        // h:=buffer[j];
