...
macro_rules! Display_glue_p {
    ($globals:expr, $p:expr) => {{
        ...
        // else  begin print_esc("glue");
            ...
            print_esc($globals, strpool_str!("glue"));
            ...
            if subtype!($globals, $p as pointer) as integer != glue_node_subtype::normal as integer
            {
                ...
                if (subtype!($globals, $p as pointer) as integer) < glue_node_subtype::cond_math_glue as integer {
                ...
                else if (subtype!($globals, $p as pointer) as integer) == glue_node_subtype::cond_math_glue as integer {
                    print_esc($globals, strpool_str!("nonscript"));
                }
                ...
                != glue_node_subtype::cond_math_glue as integer
            {
                ...
                if (subtype!($globals, $p as pointer) as integer) < glue_node_subtype::cond_math_glue as integer {
                    print_spec($globals, glue_ptr!($globals, $p as pointer) as integer, str_number::zero());
                }
                // else print_spec(glue_ptr(p),"mu");
                else {
                    print_spec($globals, glue_ptr!($globals, $p as pointer) as integer, strpool_str!("mu"));
                }
                // end;
            }
            // end
