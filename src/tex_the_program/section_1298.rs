...
pub(crate) macro Complete_a_potentially_long_show_command($globals:expr) {{
    // end_diagnostic(true); print_err("OK");
    end_diagnostic($globals, true);
    print_err!($globals, crate::strpool_str!("OK"));
    // @.OK@>
    ...
    if $globals.selector == term_and_log && tracing_online!($globals) <= 0 {
        ...
        print(
            $globals,
            crate::strpool_str!(" (see the transcript file)").get() as _,
        );
