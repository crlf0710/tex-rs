...
pub(crate) macro Scan_preamble_text_until_cur_cmd_is_tab_mark_or_car_ret__looking_for_changes_in_the_tabskip_glue__append_an_alignrecord_to_the_preamble_list($globals:expr) {{
    // @<Scan the template \<u_j>, putting the resulting token list in |hold_head|@>;
    crate::section_0783::Scan_the_template_u_j__putting_the_resulting_token_list_in_hold_head!(
        $globals
    );
    ...
    crate::section_0784::Scan_the_template_v_j__putting_the_resulting_token_list_in_hold_head!(
        $globals
    );
