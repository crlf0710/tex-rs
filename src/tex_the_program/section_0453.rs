...
pub(crate) macro Scan_units_and_set_cur_val_to_x_dot_cur_val_f_2_16_where_there_are_x_sp_per_unit__goto_attach_sign_if_the_units_are_internal {
    ($globals:expr, $mu:expr, $inf:expr, $f:expr, $lbl_attach_sign:lifetime) => {{
        ...
        if $inf {
            crate::section_0454::Scan_for_f_fil_units_goto_attach_fraction_if_found!($globals, 'attach_fraction);
        }
        // @<Scan for \(u)units that are internal dimensions;
        //   |goto attach_sign| with |cur_val| set if found@>;
        crate::section_0455::Scan_for_u_units_that_are_internal_dimensions__goto_attach_sign_with_cur_val_set_if_found!
            ($globals, $f, $mu, $lbl_attach_sign);
        // if mu then @<Scan for \(m)\.{mu} units and |goto attach_fraction|@>;
        if $mu {
            crate::section_0456::Scan_for_m_mu_units_and_goto_attach_fraction!($globals, 'attach_fraction);
        }
        // if scan_keyword("true") then @<Adjust \(f)for the magnification ratio@>;
        if scan_keyword($globals, crate::strpool_str!("true"))? {
            todo!("magnification");
        }
        // @.true@>
        // if scan_keyword("pt") then goto attach_fraction; {the easy case}
        if scan_keyword($globals, crate::strpool_str!("pt"))? {
            crate::goto_forward_label!('attach_fraction);
        }
        // @.pt@>
        // @<Scan for \(a)all other units and adjust |cur_val| and |f| accordingly;
        //   |goto done| in the case of scaled points@>;
        crate::section_0458::Scan_for_a_all_other_units_and_adjust_cur_val_and_f_accordingly__goto_done_in_the_case_of_scaled_points!
            ($globals, $f, 'done);
        }
