...
pub(crate) macro Report_an_underfull_vbox_and_goto_common_ending__if_this_box_is_sufficiently_bad($globals:expr, $x:expr, $lbl_common_ending:lifetime) {{
    ...
    if $globals.last_badness > vbadness!($globals) {
        ...
        print(
            $globals,
            crate::strpool_str!(" \\vbox (badness ").get() as _,
        );
