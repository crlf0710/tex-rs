...
macro_rules! Dump_the_hyphenation_tables {
    ($globals:expr) => {{
        // dump_int(hyph_count);
        ...
        print($globals, strpool_str!(" hyphenation exception").get() as _);
        ...
        for k in (0..=255).rev() {
            if $globals.trie_used[k] > min_quarterword {
                ...
                print($globals, strpool_str!(" for language ").get() as _);
